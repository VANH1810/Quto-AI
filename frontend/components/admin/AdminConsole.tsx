...
import type { AdminAlert, AdminCommune, AuditEntry, CommuneRisk, DeliveryIncident, Speaker, UnreachedRecipients } from "@/types/admin-console";
...
export type AdminPage = "overview" | "alerts" | "delivery" | "sos" | "speakers" | "audit";
const nav: Array<{ page: AdminPage; href: string; label: string; icon: typeof Map }> = [{ page: "overview", href: "/admin", label: "Tổng quan", icon: Map }, { page: "alerts", href: "/admin/alerts", label: "Cảnh báo", icon: AlertTriangle }, { page: "delivery", href: "/admin/delivery", label: "Gửi tin & sự cố", icon: Send }, { page: "sos", href: "/admin/sos", label: "SOS khẩn cấp", icon: Siren }, { page: "speakers", href: "/admin/speakers", label: "Loa truyền thanh", icon: Volume2 }, { page: "audit", href: "/admin/audit", label: "Nhật ký tương tác", icon: ClipboardList }];
...
export function AdminConsole({ page, alertId, sosId }: { page: AdminPage; alertId?: string; sosId?: string }) {
  ...
  const load = useCallback(async (activeToken?: string, signal?: AbortSignal) => {
    ...
    try {
      const [nextScope, nextRisks, nextIncidents, nextAlerts, nextSpeakers, nextSos] = await Promise.all([
        ...
        isOverview || page === "alerts" ? getScopedRisks(activeToken, signal) : Promise.resolve([]),
  ...
  const handleOpenIncident = useCallback(async (incident: DeliveryIncident) => {
    ...
    try {
      setRecipients(await getUnreachedRecipients(incident.alertId, token));
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Không thể tải người chưa nhận.");
    }
  }, [token]);
  const handleRetry = useCallback(async (id: string) => {
    await retryAlert(id, token);
    setNotice("Đã gửi lại các kênh lỗi.");
  }, [token]);
  if (authState === "checking") return <AdminSessionLoader />;
  if (authState === "unauthenticated") return <AdminLoginGate onAuthenticate={authenticate} />;
  return <main className="admin-console"><header className="admin-public-header"><Link className="admin-public-brand" href="/" aria-label="Về bản đồ cảnh báo thiên tai"><Image src="/figma/dien-bien-science-logo.png" width={99} height={99} priority alt="Biểu trưng Sở Khoa học và Công nghệ Điện Biên" /><span><small>Sở Khoa học &amp; Công nghệ Điện Biên</small><strong>Cảnh báo thiên tai tỉnh Điện Biên</strong></span></Link><nav className="admin-public-nav" aria-label="Điều hướng hệ thống"><Link href="/">Bản đồ cảnh báo</Link><span aria-current="page">Điều hành PCTT</span></nav></header><aside className={`admin-nav ${menuOpen ? "is-open" : ""}`} aria-label="Điều hướng quản trị"><div className="nav-brand"><ShieldCheck size={22} /><span>Điều hành PCTT</span><button aria-label="Đóng menu" onClick={() => setMenuOpen(false)}><X size={20} /></button></div><nav>{nav.map((item) => { const Icon = item.icon; return <Link key={item.page} href={item.href} className={page === item.page ? "active" : ""} onClick={() => setMenuOpen(false)}><Icon size={18} />{item.label}</Link>; })}</nav><div className="nav-session"><span>{admin?.full_name}</span><button onClick={logout}><LogOut size={16} /> Đăng xuất</button></div></aside><div className="console-backdrop" hidden={!menuOpen} onClick={() => setMenuOpen(false)} />
...
function AdminSessionLoader() {
  return <main className="admin-console admin-login-shell"><header className="admin-public-header"><Link className="admin-public-brand" href="/" aria-label="Về bản đồ cảnh báo thiên tai"><Image src="/figma/dien-bien-science-logo.png" width={99} height={99} priority alt="Biểu trưng Sở Khoa học và Công nghệ Điện Biên" /><span><small>Sở Khoa học &amp; Công nghệ Điện Biên</small><strong>Cảnh báo thiên tai tỉnh Điện Biên</strong></span></Link><nav className="admin-public-nav" aria-label="Điều hướng hệ thống"><Link href="/">Bản đồ cảnh báo</Link><span aria-current="page">Điều hành PCTT</span></nav></header><section className="admin-session-loader" role="status" aria-live="polite"><ShieldCheck size={30} /><strong>Đang mở Điều hành PCTT…</strong><span>Kiểm tra phiên đăng nhập an toàn.</span></section></main>;
}
...
const PageContent = memo(function PageContent(props: PageContentProps) {
  const { page } = props; if (page === "overview") return <Overview {...props} />; if (page === "alerts") return props.alertId ? <AlertDetail alert={props.alerts.find((item) => item.id === props.alertId)} onRetry={props.onRetry} /> : <Alerts alerts={props.alerts} risks={props.risks} />; if (page === "delivery") return <Delivery incidents={props.incidents} onProcess={props.onProcess} />; if(page==="sos")return <SosPage items={props.sos} detailId={props.sosId} token={props.token}/>; if (page === "speakers") return <Speakers items={props.speakers} />; return <Audit />;
});
function Overview({ scope, risks, incidents, alerts, sos, highestAlert, onlineSpeakers, sentToday, onProcess }: PageContentProps) { const urgent=sos.filter(x=>x.status==="NEW"); return <><section className="overview-metrics"><Metric label="Cảnh báo đang hiệu lực" value={alerts.filter((item) => ["pending_approval", "partial_failed"].includes(item.status)).length} tone="critical" /><Metric label="Xã có nguy cơ" value={risks.filter((item) => item.risk_level > 0).length} suffix={`/${scope.length}`} tone="warning" /><Metric label="SOS chờ xử lý" value={urgent.length} tone="critical" /><Metric label="Loa trực tuyến" value={onlineSpeakers} suffix="/3" tone="safe" /><Metric label="Tin gửi hôm nay" value={sentToday} /></section>{highestAlert ? <section className="priority-alert"><div><span>Cao nhất cần phê duyệt · Cấp {highestAlert.event.risk_level}</span><h2>{highestAlert.bulletins[0]?.title ?? hazardLabel(highestAlert.event.hazard)}</h2><p>{highestAlert.event.commune_name} · phát hành {formatDate(highestAlert.created_at)}</p></div><Link className="primary-link" href={`/admin/alerts/${highestAlert.id}`}>Xem và phê duyệt <ChevronRight size={17} /></Link></section> : <section className="console-empty"><ShieldCheck size={26} /><h2>Không có cảnh báo chờ phê duyệt</h2><p>Hệ thống đang theo dõi các xã trong phạm vi được phân quyền.</p></section>}{urgent.length>0&&<section className="console-card sos-summary"><header><h2><span>SOS</span> mới hoặc nghiêm trọng</h2><Link href="/admin/sos">Xem tất cả SOS</Link></header><SosRows items={urgent.slice(0,3)}/></section>}<div className="overview-grid overview-grid--single"><section className="console-card contact-panel"><header><h2>Cần liên hệ trực tiếp</h2><Link href="/admin/delivery">Xem tất cả</Link></header><IncidentRows incidents={incidents.slice(0, 3)} onProcess={onProcess} /></section></div></> }
