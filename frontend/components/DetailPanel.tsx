...
import type { CommuneAlert, CommuneCenter, SelectedPlace, Shelter, UserPosition } from "@/types";
...
export function DetailPanel({ selection, alerts, communes, shelters, userPosition, onSelectShelter }: DetailPanelProps) {
  ...
  if (selectedShelter) {
    ...
    return (
      <aside className="detail-panel">
        <div className="detail-top shelter"><span className="detail-icon"><Building2 size={22} /></span><div><small>Điểm trú ẩn an toàn</small><h2>{selectedShelter.name}</h2></div></div>
        <div className="detail-body">
          <div className="shelter-facts"><span><MapPinned size={18} /><b>Địa chỉ</b><small>{selectedShelter.address}</small></span><span><Users size={18} /><b>Sức chứa dự kiến</b><small>{selectedShelter.capacity} người</small></span></div>
          ...
          <button className="primary-action"><Navigation size={18} /> Xem đường đến điểm trú ẩn</button>
  ...
  if (!selectedAlert || !selectedCommune) {
    return (
      <aside className="detail-panel empty-detail"><ShieldAlert size={32} /><h2>Chọn một khu vực</h2><p>Chạm vào xã hoặc marker trên bản đồ để xem cảnh báo và hướng dẫn an toàn.</p></aside>
    );
  }
  ...
  return (
    <aside className="detail-panel">
      <div className="detail-top" style={{ background: `linear-gradient(135deg, ${risk.color}, ${risk.color}d8)` }}>
        ...
        <div><small>{risk.label}</small><h2>{selectedCommune.name}</h2><p>{selectedCommune.district}</p></div>
      </div>
      <div className="detail-body">
        ...
        {communeShelters[0] && <button className="nearest-shelter" onClick={() => onSelectShelter(communeShelters[0].id)}><Building2 size={20} /><span><small>Điểm trú ẩn đề xuất</small><strong>{communeShelters[0].name}</strong><em>Sức chứa {communeShelters[0].capacity} người</em></span><ArrowRight size={18} /></button>}
