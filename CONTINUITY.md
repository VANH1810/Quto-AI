...
- **State:**
  ...
  - **Now:** Render cần đổi Start Command → `uvicorn app.main:app --host 0.0.0.0 --port $PORT` (bỏ --reload) + set env SUPABASE_URL/KEY/DB_BACKEND. Chạy `db/schema.sql` để data bền qua restart (nếu không, mỗi lần restart auto-seed lại). Chưa commit.
